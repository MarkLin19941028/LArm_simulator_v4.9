...
class PREGenerator:
    ...
    def _run_core_simulation(self, recipe, config=None, progress_widgets=None, fast_mode=False, play_speed_multiplier=1.0):
        ...
        defectmap_cali = config.get('PRE_DEFECT_CALI', 10.0) # 更新預設值
        ...
        for i, geo in ARM_GEOMETRIES.items():
            # LArm: 補齊 p_start, p_end
            headless_arms[i] = DispenseArm(i, geo['pivot'], geo['home'], geo['length'], geo['p_start'], geo['p_end'], None, None)

        water_params = self.app._get_water_params() if hasattr(self, 'app') else {'viscosity': 1.0, 'surface_tension': 72.8, 'evaporation_rate': 0.0}
