...
class AutoTunerGUI:
    ...
    def load_csv(self):
        ...
        try:
            ...
            self.spline_target = UnivariateSpline(self.exp_radius, self.exp_values, k=2, s=1.0)
            
            # 檢查 0mm 處的數值，若仍不合理，可手動強制修正邊界值
            # self.spline_target.set_type(ext=3) # 限制外推
    ...
    def finish_tuning(self, best_params, best_mse, best_sim_radial, v_target_smooth):
        ...
        plt.scatter(self.exp_radius, self.exp_values, color='red', label='Exp Data (Discrete)')
        plt.plot(self.target_radius_range, v_target_smooth, 'orange', linestyle='--', label='Spline Target')
        ...
        plt.xlabel("Radius (mm)"); plt.ylabel("Etching Amount"); plt.legend(); plt.grid(True)
        plt.show()
