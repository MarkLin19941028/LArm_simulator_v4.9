            ...
            if (sim_clock - last_progress_display_time >= progress_display_interval) or snapshot.get('is_finished'):
                if progress_widgets:
                    try:
                        ...
                        p_label.config(text=f"Etching Calculation (Accelerated): {sim_clock:.1f}s / {total_duration:.1f}s ({percent:.0f}%)")
