...
def fast_accumulate(coords, rot_matrix, heatmap, dt, grid_size, range_min, range_max):
    """
    使用 Numba JIT 編譯的極速累積函數。
    一次完成：座標旋轉 -> 邊界檢查 -> 網格索引計算 -> 累加
    """
    ...
    for i in range(n_particles):
        ...
        # 1. 旋轉座標 (手動展開矩陣乘法比 np.dot 在迴圈內更快)
        # rot_back * [x, y]^T
        rx = r00 * x + r01 * y
        ry = r10 * x + r11 * y
    ...
    def generate(self, recipe, filepath, config=None, progress_widgets=None):
        ...
        while True:
            ...
            if coords_array.size > 0:
                ...
                # 預先計算 sin/cos
                c, s = math.cos(-rad_wafer), math.sin(-rad_wafer)
                rot_back = np.array([[c, -s], [s, c]], dtype=np.float64)
